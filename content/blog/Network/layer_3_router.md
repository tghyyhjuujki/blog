...
인터넷 제어 메세지 프로토콜로, 에러 보고 프로토콜이라고도 부른다. ICMP패킷은 일반적인 패킷과 다르게 IP헤더와 ICMP 메세지로 구성되어있다.

크게 쿼리메세지와 에러메세지가 있다. Type의 3, 5,11,12는 에러메세지고 나머지는 쿼리 메세지다.

![image-20201116164235359](layer_3_router.assets/image-20201116164235359.png)

<br/>

### TTL(Time To Live)

ICMP 패킷의 IP헤더부분에는 TTL(생존시간)항목이 존재한다. IP데이터그램의 TTL은 라우터를경유할 때마다 1씩 줄어들고 0이 되면 데이터그램은 파기된다. 잘못 되었을 때 네트워크에 떠돌아 다닐 경우, 방해만 되니까 시간을두어 파기시키는 것이다.

<br/>

### Echo

Type 0, 8번은 에코 타입이다. 송신측은 에코 요청을 보내고 그것을 수신한 컴퓨터는 에코 응답을 돌려보내는 구조를 의미한다. 에코가 정상적으로 작동하면, 송신처와 수신처 사이에서 데이터교환이 가능함을 뜻한다. 보통  ping 명령어로 사용함.

<br/>

### Time Exceeded(시간초과)

TTL은 끊어진패킷을파괴한다. 이 때, 파기한 것을 통지하는 메세지가 11번인 Time Exceeded이다. 

<br/>

### traceroute

Time Exceeded를 사용하여 네트워크 경로를 체크하는 명령어가 바로 tracetoute이다. 수신처까지 경유하는 라우터를 알려준다. 이 방법으로 일부러 에러메세지를 보내 라우터 경로를 하나씩 체크할 수 있다.

하지만 이 명령어는 크래킹에 사용될 수도 있다. 라우터의 IP를 알면 그곳을 공격할 수 있고, 네트워크 전체에 피해를 입힐 수 있다.